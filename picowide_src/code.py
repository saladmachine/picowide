...
# --- GLOBAL PLACEHOLDERS FOR SPHINX TO SEE ---
# Sphinx needs 'server' to exist at the top level for @server.route decorators
# We initialize 'server' with a dummy object for documentation purposes.
# 'pool' is also set globally as None or a dummy if needed by other global definitions
# The real instances of 'server' and 'pool' are created within main() for actual runtime.
class _DummyServer:
    """A dummy class to provide a .route() method for Sphinx's autodoc."""
    def route(self, path, methods=["GET"]):
        def decorator(func):
            return func # Return the function untouched for Sphinx to document
        return decorator
server = _DummyServer() # Initialize server globally with the dummy object
pool = None # Initialize pool globally as None
...
# Blinky timing globals - must be outside update_blinky
last_blink = time.monotonic()
led_state = False
blinky_enabled = False # Initial state
BLINK_INTERVAL = 0.25 # Default value, updated from config in main()
...
def update_blinky():
    ...
    :rtype: None
    """
    global last_blink, led_state, blinky_enabled, BLINK_INTERVAL # Ensure BLINK_INTERVAL is global here
    ...
    if current_time - last_blink >= BLINK_INTERVAL: # Use the global BLINK_INTERVAL
...
def get_startup_log_route(request: Request): # Renamed to avoid clash with global var
    """
...
# =============================================================================
# MAIN EXECUTION BLOCK (Wrapped for Sphinx compatibility)
# =============================================================================
def main():
    """
    Initializes and runs the Picowide web-based development environment.

    This function sets up WiFi, configures the network, starts the web server,
    and enters the main polling loop. This code is only executed when the
    script is run directly (e.g., on the Pico W), not when imported by Sphinx.
    """
    global blinky_enabled, WIFI_TIMEOUT_SECONDS, last_activity_time, last_timeout_check_log_time, ap_is_off_and_logged, timeout_disabled, server, pool, led, BLINK_INTERVAL # Added server and pool to globals
    
    # --- BULLETPROOF: Config Import with guaranteed fallback ---
    # Initialize defaults first - ALWAYS have working config
    class Config: # Moved Config class definition here or ensure it's globally available
        WIFI_SSID = "Picowide"
    ...
    try:
        ...
        # Ensure 'config' module exists for this import, or it will fail
        # This import will look for 'config.py' in the same directory as code.py
        # or in sys.path. If config.py is also in picowide_src, that's fine.
        # If it's in the root (~/picowide/config.py), Python should find it.
    ...
    # Update global BLINK_INTERVAL based on loaded config
    BLINK_INTERVAL = config.BLINK_INTERVAL # Update global BLINK_INTERVAL here too
    ...
    if not ap_success:
        config_failed = True
        config.BLINK_INTERVAL = 0.10 # This won't update global BLINK_INTERVAL without reassign
        BLINK_INTERVAL = config.BLINK_INTERVAL # Update global BLINK_INTERVAL here too
    ...
    # Initialize server
    pool = socketpool.SocketPool(wifi.radio) # Now defined within main
    server = Server(pool, "/", debug=False) # Now defined within main

    # Re-route server handlers after server is defined
    # These will dynamically add routes at runtime.
    server.add_route("/", serve_index)
    server.add_route("/styles.css", serve_styles)
    server.add_route("/run-blinky", run_blinky)
    server.add_route("/toggle-hotspot-control", toggle_hotspot_control)
    server.add_route("/power-save", power_save_mode)
    server.add_route("/list-files", list_files)
    server.add_route("/select-file", select_file)
    server.add_route("/open-file", open_file)
    server.add_route("/run-monitor", run_monitor)
    server.add_route("/get-console", get_console)
    server.add_route("/create-file", create_file)
    server.add_route("/save-file", save_file)
    server.add_route("/delete-file", delete_file)
    server.add_route("/startup-log", get_startup_log_route) # Use get_startup_log_route
...
# This is the standard Python entry point. It calls main() only when the script is run directly.
